    ...
    <div class="container">
      <div class="row">
        <div class="col-12">
          ...
          <?php 
            $sql = " SELECT * FROM tb_artikel WHERE id=$_GET[id] " ;
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) { ?>
                      
                      
                      <h6>Post on <a href="#" class="post-date"><?php $originalDate = $row['tgl'];
                              echo $newDate = date( "d-M-Y", strtotime($originalDate)) ?></a> / <a href="#" class="post-author">Admin</a></h6>
                      <h2 class="post-title"><?php echo $row['judulartikel']; ?></h2>
                      <div class="col-lg-8">
                          <img class="w-100 mb-30" src="img/bg-img/<?php echo $row['gambar'] ?>" alt="">
                      </div>
